...
import {
  ContentInner, WorkArea, ResultArea, ButtonArea, LeftButtonArea, RightButtonArea,
...
function MenuMgmt() {
  ...
  useEffect(() => {
    if (menuGrid) {
      ...
      setViewInfo(activeViewId, 'NO_SEARCHAREA', true)
  ...
  function setGridFieldAndColumn(dataProvider, gridView) {
    ...
    let columns = [
      ...
      {
        name: "filePath", fieldName: "filePath",
        header: { text: transLangKey("MENU_FILE_PATH") },
        styleCallback: function (grid, dataCell) {
          let ret = {}
          let insertFlag = grid.getValue(dataCell.index.itemIndex, "insertFlag");
          let parentId = grid.getValue(dataCell.index.itemIndex, "icon");
          if (insertFlag !== 'Large' && insertFlag !== "Medium") {
            ret.readonly = false;
            ret.editable = true;
            ret.styleName = 'editable-text-column';
          } else {
    ...
    gridView.onContextMenuPopup = function (grid, x, y, elementName) {
      if (elementName) {
        ...
        } else if (selectRow.parentId !== null && rowState ? insertFlag === 'Medium' : hasChildren) {
        } else {
          return false;
        }
      }
    ...
    gridView.onContextMenuItemClicked = function (grid, item, clickData) {
      ...
      if (item.label === transLangKey("INSERT_MENU")) {
        dataProvider.addChildRow(selectRow['__rowId'], ['', '', menuId, '', null, defaultSeq, true, '', null, 'Medium'], -1, true);
      } else if (item.label === transLangKey("INSERT_UI")) {
        dataProvider.addChildRow(selectRow['__rowId'], ['', '', menuId, '', '', defaultSeq, true, null, null, 'Small']);
      }
  ...
  async function loadData(current) {
    ...
    let ancs = []
    if (current) {
      if (gridView.getAncestors(current.itemIndex, false).length > 0) {
        ancs = gridView.getAncestors(current.itemIndex, false)
      }
    }
    ...
    zAxios.get('system/menus?all-menus=true', { waitOn: false })
      .then(function (res) {
        ...
        if (current !== undefined) {
          (async () => {
            await ancs.reverse().forEach(pInx => {
                gridView.expand(pInx);
            })

            if (current !== undefined) {
              await gridView.setCurrent({ itemIndex: current.itemIndex, column: current.fieldName });
              await gridView.setFocus();
            }
          })().catch(err => {
            console.log(err)
          })
  ...
  function insertMenu() {
    ...
    menuGrid.dataProvider.addChildRow(0, ['', '', null, '', null, defaultSeq, true, '', '', 'Large'], -1, true);
  }
  function deleteMenu() {
    menuGrid.gridView.commit(true);
            ...
            zAxios({
              ...
              .then(function (response) {
                if (response.status === HTTP_STATUS.SUCCESS) {
                  ...
                  loadData(menuGrid.gridView.getCurrent());
                }
              })
              .catch(function (err) {
                console.log(err);
      ...
      showMessage(transLangKey('MSG_CONFIRM'), transLangKey('MSG_SAVE'), function (answer) {
        if (answer) {
          ...
          changes.forEach(function (row) {
            ...
            if (!changeRow.insertFlag || changeRow.insertFlag !== 'Large' && changeRow.parentId.length === 0) err = "4";
          });

          menuGrid.dataProvider.getAllStateRows().created.forEach(function (row) {
            let changeRow = menuGrid.dataProvider.getJsonRow(row);
            if (changeRow.insertFlag !== 'Small' && menuGrid.dataProvider.getChildren(row) === null) {
              err = "5";
            }
          });
          ...
          changes.forEach(function (row) {
            let changeRow = menuGrid.dataProvider.getJsonRow(row);
            if (changeRow.insertFlag === 'Small') {
              changeRow.path = getAllPath(row, changeRow.path);
              changeRow.type = changeRow.type === "true" ? "true" : null;
