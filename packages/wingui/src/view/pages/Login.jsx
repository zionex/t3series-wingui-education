...
import {
  Alert, Box, IconButton, InputAdornment, Snackbar, Typography, FormControl, FormGroup, FormControlLabel,
  Checkbox
} from "@mui/material";
...
function Login(props) {
  ...
  const [isLogin, login, passwordExpired, setIsLogin, setInitApp] = useUserStore(state => [state.isLogin, state.login, state.passwordExpired, state.setIsLogin, state.setInitApp])
  ...
  return (
    <>
      ...
      <Snackbar anchorOrigin={{ vertical: 'top', horizontal: 'right' }} autoHideDuration={3000} open={toast}>
        <Alert onClose={hideToast} variant="outlined" severity="error" sx={{ width: "100%" }}>
          {/* {transLangKey('MSG_LOGIN_FAIL')} */}
          {transLangKey(resMessage)}
        </Alert>
      </Snackbar>
    </>
  )
