...
### RealGrid DataProvider의 DataField 자료형 
RealGrid의 `DataProvider`에서 `DataField`는 다음 네 가지 자료형 중 하나로 설정할 수 있습니다
...
- `grid1Items`는 그리드의 컬럼을 정의합니다. 각 컬럼은 `name`, `dataType`, `headerText`, `visible`, `editable`, `width`, `autoFilter`, `editableNew`, `autoFilter`, `textAlignment`, `editableNew` 등의 속성을 가집니다.
