    ...
    return this.each(function () {
      ...
      var methods = {
        init: function () {
          ...
          //CANCEL BUTTON
          var cancelBtnDiv = $('<div class="' + elemClass + '_text" style="color:' + color + '"></div>');
          var cancelButton = $('<button type="button" style="padding: 6px 12px; opacity: 0; margin: 4px 2px;border-radius: 2px;background-color: #ffd3d3;border: none"> Cancel </button>');
          if (_options.onCancelAxios) {
            cancelButton.on('click', function () { waitMeClose(); _options.onCancelAxios({ config: _options.config, message: _options.cancelAxiosMsg ? _options.cancelAxiosMsg : 'Canceled by User' }) })
          } else {
            cancelButton.on('click', function () { waitMeClose(); })
          }

          cancelBtnDiv.append(cancelButton);
          setTimeout(()=>{
            cancelButton.animate({ opacity: 0.8, }, 4000)
            waitMeDivObj.append(cancelBtnDiv);
          }, 60000) //취소버튼은 60초 후에 나타나게 함
